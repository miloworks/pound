...
#define POUND_TAB_STOP 8
#define POUND_QUIT_TIMES 3
...
int editor_row_rx_to_cx(erow *row, int rx) {
	...
	for (cx = 0; cx < row->size; cx++) {
		if (row->chars[cx] == '\t')
			cur_rx += (POUND_TAB_STOP - 1) - (cur_rx % POUND_TAB_STOP);
		cur_rx++;
...
void editor_update_row(erow *row) {
	...
	for (j = 0; j < row->size; j++) {
		if (row->chars[j] == '\t') {
			row->render[idx++] = ' ';
			while (idx % POUND_TAB_STOP != 0) row->render[idx++] = ' ';
		} else {
...
void editor_insert_newline() {
	if (E.cx == 0) {
		...
		erow *row = &E.row[E.cy];
		editor_insert_row(E.cy, &row->chars[E.cx], row->size - E.cx);
		row = &E.row[E.cy];
		row->size = E.cx;
		row->chars[row->size] = '\0';
		editor_update_row(row);
...
void editor_draws_rows(struct abuf *ab) {
	int y;
...
void editor_process_keypress() {
	static int quit_times = POUND_QUIT_TIMES;
